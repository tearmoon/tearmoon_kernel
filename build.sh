...
[[ ! -z "$KSU" && -f "arch/arm64/configs/vendor/ginkgo-perf_defconfig" ]] || exit 1
SECONDS=0 # builtin bash timer
if [ "${KSU}" = "true" ]; then
  ZIPNAME="TearMoon-KSU@${HEADS}-$(date +"%Y%m%d").zip"
  sed -i "s|CONFIG_LOCALVERSION=.*|CONFIG_LOCALVERSION=\"-TearMoon-KSU@${HEADS}\"|g" "arch/arm64/configs/vendor/ginkgo-perf_defconfig"
else
...
CLANG_DIR="$HOME/toolchain/linux-x86/clang-r510928"
...
export KBUILD_BUILD_USER="MiaLuna"
export KBUILD_BUILD_HOST="Tearmoon"
...
if [ -f "out/arch/arm64/boot/Image.gz-dtb" ] && [ -f "out/arch/arm64/boot/dtbo.img" ]; then
  ...
  sed -i "s|kernel.string=.*|kernel.string=TearMoon Kernel by taamarin.t.me @ telegram|g" ./anykernel.sh
